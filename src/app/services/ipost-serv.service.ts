...
export class IpostServService {
  // array di podt completo non mischiato
  postsOrd: iPosts[] = [
    {
      id: 1,
  ...
  // array di post inattivi
  inactivePosts!: iPosts[];
  ...
  // array di post completo mischiato
  posts: iPosts[] = this.shuffle(this.postsOrd);

  // metodo per che torna un array di post inattivi mischiati
  getInactivePost = (param: iPosts[]) => {
    this.inactivePosts = param.filter((post) => (post.active ? false : true));
    return this.shuffle(this.inactivePosts);
  };

  // metodo per che torna un array di post attivi mischiati
